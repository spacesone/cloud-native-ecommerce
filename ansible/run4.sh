...
# update MONGO_URL
yq -y ".data.MONGO_URL = \"mongodb://$(jq -r '.mongodb_vm_external_ip.value' ../infrastructure/output.json):27017\"" ../k8s/configmap.yaml | sponge ../k8s/configmap.yaml
...
# yq -y ".data.JWT_ISSUER_URI = \"http://$(jq -r '.mongodb_vm_external_ip.value' ../infrastructure/output.json):8080/realms/cloud-native-ecommerce\"" ../k8s/configmap.yaml | sponge ../k8s/configmap.yaml

# # update JWT_SET_URI
# yq -y ".data.JWT_SET_URI = \"http://$(jq -r '.mongodb_vm_external_ip.value' ../infrastructure/output.json):8080/realms/cloud-native-ecommerce/protocol/openid-connect/certs\"" ../k8s/configmap.yaml | sponge ../k8s/configmap.yaml